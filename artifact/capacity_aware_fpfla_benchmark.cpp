...
int main(){
 mt19937_64 rng(20260905), order_rng(772019);
 ...
 cout<<"inst,ptype,rep,n,m,cargo_lb,init_ub,opt,base_us,ub_us,direct_us,cap_us,base_pairs,ub_pairs,direct_pairs,cap_pairs,base_popped,cap_popped,cap_pruned,maxfront_base,maxfront_cap,ub_feasible\n";
 for(auto path:files){Inst I=load_evrp(path);ResetMat R(I);StationMat M(I);string name=filesystem::path(path).filename().string();
  for(int typ=0;typ<2;typ++)for(int rep=0;rep<20;rep++){
   auto p=knn(I,rng,typ==0?3:I.n);auto S=prep(I,M,p);
   Stats chk;double opt=solve(I,R,p,S,INF,NO_LB,chk);if(opt>=INF/2)continue; bool uf=S.feasibleUB<INF/2;double U=uf?S.feasibleUB:opt;
   vector<vector<long long>> times(4); vector<Stats> stat(4); vector<int> jobs; for(int rr=0;rr<3;rr++)for(int mm=0;mm<4;mm++)jobs.push_back(mm);shuffle(jobs.begin(),jobs.end(),order_rng);
   for(int job:jobs){Stats z;double val;
     if(job==0)val=solve(I,R,p,S,INF,NO_LB,z); else if(job==1)val=solve(I,R,p,S,U,NO_LB,z); else if(job==2)val=solve(I,R,p,S,U,DIRECT_LB,z); else val=solve(I,R,p,S,U,CAP_LB,z);
     if(abs(val-opt)>1e-6){cerr<<setprecision(12)<<"MISMATCH "<<name<<" job="<<job<<" val="<<val<<" opt="<<opt<<"\n";return 3;}
     if(times[job].empty())stat[job]=z;times[job].push_back(z.us);
   }
   long long med[4];for(int j=0;j<4;j++){sort(times[j].begin(),times[j].end());med[j]=times[j][1];}
   cout<<name<<","<<(typ==0?"knn3":"random")<<","<<rep<<","<<I.n<<","<<I.m<<","<<S.cargoLB<<","<<U<<","<<opt<<","<<med[0]<<","<<med[1]<<","<<med[2]<<","<<med[3]<<","<<stat[0].pairs<<","<<stat[1].pairs<<","<<stat[2].pairs<<","<<stat[3].pairs<<","<<stat[0].popped<<","<<stat[3].popped<<","<<stat[3].lbpr<<","<<stat[0].maxfront<<","<<stat[3].maxfront<<","<<(uf?1:0)<<"\n";
  }
 }
}
